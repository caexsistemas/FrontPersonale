...
@Component({
  selector: "app-CollaboratorsPermission",
  templateUrl: "./CollaboratorsPermission.component.html",
  styleUrls: ["./CollaboratorsPermission.component.css"],
})
export class CollaboratorsPermissionComponent implements OnInit {
    ...
    this.WebApiService.getRequest(this.endpoint, {
      ...
      (data) => {
        if (data.success == true) {
          ...
          !(this.rol == 5 || this.rol == 7 || this.rol == 20 || this.rol == 1)
            ? (this.chekPdf = false)
            : (this.chekPdf = true);
          this.loading = false;
        } else {
  ...
  option(action, codigo = null, estado = null) {
    ...
    switch (action) {
      case "create":
        this.loading = true;
        dialogRef = this.dialog.open(CollaboratorDialog, {
          data: {
            window: "create",
            codigo,
          },
  ...
  onTriggerSheetClick() {
    this.matBottomSheet.open(ReportsCollaboratorComponent);
  }
}
